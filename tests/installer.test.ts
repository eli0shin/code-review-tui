...
async function fixture(
  ...
  const root = await mkdtemp(join(tmpdir(), 'review-installer-'));
...
describe('binary installer', () => {
  for (const [system, machine, artifact] of [
    ['Linux', 'x86_64', 'review-linux-x64'],
    ['Linux', 'aarch64', 'review-linux-arm64'],
    ['Darwin', 'x86_64', 'review-darwin-x64'],
    ['Darwin', 'arm64', 'review-darwin-arm64'],
  ] as const) {
    test(`installs ${artifact} as the only payload`, async () => {
      const { home, path, downloadedUrlPath } = await fixture(system, machine);
      const installedPath = join(home, '.local/bin/review');

      expect(await runInstaller(home, path)).toEqual({
        stdout: `Installed review to ${installedPath}\n\nAdd this to your shell profile to use review:\n  export PATH="$HOME/.local/bin:$PATH"\n`,
        stderr: '',
        exitCode: 0,
      });
      expect(await readFile(downloadedUrlPath, 'utf8')).toBe(
        `https://github.com/eli0shin/code-review-tui/releases/latest/download/${artifact}`
      );
      expect(await readFile(installedPath, 'utf8')).toBe(
        'native review binary'
      );
      expect((await stat(installedPath)).mode & 0o111).not.toBe(0);
      expect(await readdir(join(home, '.local/bin'))).toEqual(['review']);
    });
  }
  ...
  for (const destinationKind of ['directory', 'symlink to a directory']) {
    test(`rejects a ${destinationKind} at the binary destination`, async () => {
      ...
      const installedPath = join(installDirectory, 'review');
      ...
      expect(await runInstaller(home, path)).toEqual({
        stdout: '',
        stderr: `Cannot install review: ${installedPath} is a directory\n`,
        exitCode: 1,
      });
  ...
  test('rejects a destination that becomes a directory during download', async () => {
    ...
    expect(await readdir(installDirectory)).toEqual(['review']);
  });
  ...
  test('preserves an existing binary when a download fails', async () => {
    ...
    const installedPath = join(installDirectory, 'review');
    await mkdir(installDirectory, { recursive: true });
    await writeFile(installedPath, 'working review binary');
    await writeFile(
      curlPath,
    ...
    expect(await readFile(installedPath, 'utf8')).toBe('working review binary');
    expect(await readdir(installDirectory)).toEqual(['review']);
  });
