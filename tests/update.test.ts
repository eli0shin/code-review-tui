...
async function temporaryDirectory(): Promise<string> {
  const path = await mkdtemp(join(tmpdir(), 'review-update-'));
...
describe('native artifact selection', () => {
  ...
  test('builds the current platform download URL without real network access', async () => {
    ...
    expect(await fetchLatestVersion()).toEqual({
      ...
      data: {
        version: '1.2.3',
        downloadUrl: `https://github.com/eli0shin/code-review-tui/releases/latest/download/review-${currentPlatform}-${currentArchitecture}`,
      },
    });
  });
});
...
describe('runtime executable selection', () => {
  test('does not treat the Bun runtime as the Review executable', () => {
    expect(getReviewExecutablePath()).toBeUndefined();
  });
});
...
describe('update command', () => {
    ...
    expect(await updateCommand('1.2.3', undefined, dependencies)).toEqual({
      ...
      outcome: {
        ...
        failure: {
          kind: 'message',
          message:
            'Cannot update from a source invocation; use the compiled Review executable',
        },
      },
    });
  ...
  test('does not download or replace an already-current version', async () => {
    ...
    expect(await updateCommand('1.2.3', '/tmp/review', dependencies)).toEqual({
      messages: [
  ...
  test('downloads beside and replaces the selected executable', async () => {
    const directory = await temporaryDirectory();
    const binaryPath = join(directory, 'review');
    await writeFile(binaryPath, 'old binary');
    const dependencies = {
      fetchLatestVersion: async () => ({
        success: true as const,
        data: { version: '1.2.4', downloadUrl: 'mock://review' },
      }),
      isNewerVersion,
      downloadBinary: async (url, targetDirectory) => {
        ...
        const temporaryPath = join(targetDirectory, '.review-update-test');
  ...
  test('cleans a failed replacement and preserves the existing target', async () => {
    const directory = await temporaryDirectory();
    const temporaryPath = join(directory, '.review-update-failed');
    const targetPath = join(directory, 'review');
    await writeFile(temporaryPath, 'new binary');
