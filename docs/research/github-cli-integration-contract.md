...
gh pr view <pull-request-url> --json additions,deletions,changedFiles,reviewDecision

# For My PRs, also request statusCheckRollup in the same per-URL call.
```
...
The search exporter does not provide file count, additions, deletions, or review decision. Enrich every complete search result with `gh pr view` by canonical URL. My PRs also requests the check rollup in that call. Run at most eight enrichment processes at once, preserve search order, and publish the Review Queue only when every item passes validation. An enrichment failure fails the complete queue load.
