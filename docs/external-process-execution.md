...
- a reachable Herdr socket whose protocol supports `session.snapshot`, `layout.apply`, `pane.get`, `pane.focus`, `pane.close`, and event subscriptions; and
...
Focus the saved Review Queue pane with `pane.focus` after completion only when an uninterrupted subscription has two validated states: the last authoritative snapshot showed that exact owned tool pane focused, no focus invalidation arrived after that baseline, reconciliation requested by a matching current-resource exit or close notice confirms that it is absent, and the latest snapshot does not show another pane, tab, or workspace focused. If the latest snapshot identifies any other focused surface, preserve it even when no focus notice has arrived yet. Herdr replay can delay that notice behind the exit notice, so the snapshot takes precedence. A background or uncertain tool exit must not steal focus.

If the saved Review Queue pane exists but `pane.focus` fails, show a recoverable Herdr control failure. If its saved pane, tab, or workspace is absent, disable launches and reconcile the Review Queue pane by stable terminal ID through a fresh snapshot. Update the saved resource IDs and retry `pane.focus` when the pane exists in a new container. Start shutdown only when the snapshot confirms that the Review Queue pane no longer exists.
